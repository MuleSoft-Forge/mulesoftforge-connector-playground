  ...
  <div class="border border-gray-300 rounded-md bg-white">
    ...
    <div class="flex items-center justify-between bg-gray-50 px-3 py-2 border-b border-gray-300">
      <div class="flex items-center gap-2">
        <span class="text-xs font-semibold text-gray-700">Task ID Params</span>
      </div>
    ...
    <div v-if="currentView === 'form'" class="p-4 space-y-4">
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800">
        <div class="flex items-start gap-2">
          ...
          <div>
            <p class="font-semibold mb-1">Task Identifier Parameters</p>
            <p class="text-[11px] leading-relaxed">
              Specify the task ID and optional metadata for task operations like cancellation or reconnection. Used for Cancel Task and Task Resubscribe operations.
            </p>
          </div>
        </div>
      </div>
      ...
      <div>
        <label class="block text-xs font-medium text-gray-700 mb-1">
          Task ID *
          <span class="text-gray-500 font-normal ml-1">(ID of the streaming task to reconnect to)</span>
        </label>
        <input
          v-model="formData.id"
          @input="syncToJson"
      ...
      <div>
        <label class="block text-xs font-medium text-gray-700 mb-1">
          Last Event ID
          <span class="text-gray-500 font-normal ml-1">(optional - resume from specific event)</span>
        </label>
        <input
          v-model="formData.lastEventId"
          @input="syncToJson"
          type="text"
          placeholder="event-xyz789"
          class="w-full px-2 py-1.5 text-xs border border-gray-300 rounded-md font-mono"
        />
        <p class="text-[10px] text-gray-500 mt-1">If provided, receives only events after this ID</p>
      </div>

      <div>
        <label class="block text-xs font-medium text-gray-700 mb-1">
          Metadata
          <span class="text-gray-500 font-normal ml-1">(optional JSON object)</span>
        </label>
        <textarea
          v-model="metadataJson"
          @input="syncMetadataToForm"
          rows="3"
          placeholder='{"clientInfo": "reconnect-attempt-1"}'
...
const formData = ref<TaskIdParams>({
  id: '',
...
function syncToJson() {
  try {
    const obj: TaskIdParams = {
      id: formData.value.id
    }
    
    if (formData.value.lastEventId) {
      obj.lastEventId = formData.value.lastEventId
    }
